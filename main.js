...
;// ./src/components/Polling/Polling.js


class Polling {
  constructor(streamMessages$, parentEl, pollingState, messagesList, subscriptionOnStream, pollingActions) {
  ...
  bindToDOM() {
    ...
    this.pollingActions = new this.pollingActions(this.container, this.pollingState, this.subscriptionOnStream, this.subscription);
  ...
  toggleMessage(e) {
    if (e.target.closest(".message-element")) {
      e.target.closest(".message-element").querySelector(".message-text-full").classList.toggle("message-text-full_hidden");
    }
  }
}
;// ./src/classes/PollingState.js
class PollingState {
  constructor() {
    this._pollingStorage = {
      messages: []
    };
  }
  get pollingStorage() {
  ...
  addMess(mess) {
    this.pollingStorage.messages = JSON.parse(localStorage.getItem("messages")).messages;
...
class StreamMessages {
  ...
  updateStreamMessages() {
    return interval(30000).pipe(mergeMap(() => {
      return ajax.getJSON(this.url).pipe(map(messResponse => messResponse.messages), catchError(err => {
        console.error(err);
        return of([]);
      }));
    }));
...
;// ./src/utils/clipSubject.js
function clipSubject(string, length) {
...
class MessagesList {
  ...
  addMess(data) {
    ...
    const messageDemoContainer = new Div({
      class: "message-demo-container"
    }).element;
    ...
    if ([...this.listMessages.querySelectorAll(".message-element")].length > 1) {
      messageElement.style.borderTop = "none";
    }
  }
}
;// ./src/classes/SubscriptionOnStream.js
class SubscriptionOnStream {
...
;// ./src/components/PollingActions/PollingActions.js


class PollingActions {
  ...
  bindToDOM() {
    ...
    this.btnsContainer.append(this.btnGoMessages, this.btnStopeMessages, this.btnClearMessages);
  ...
  eventsBtns(e) {
    ...
    if (e.target.closest(".btn-clear-messages")) {
      ...
      if ([...listMessages.querySelectorAll(".message-element")].length > 0) {
        [...listMessages.querySelectorAll(".message-element")].forEach(messEl => {
          messEl.remove();
        });
      }
    }
...
const url = `${environment}/messages/unread`;
document.addEventListener("DOMContentLoaded", () => {
  const app = document.querySelector("#app");
  const streamMessages$ = new StreamMessages(url).updateStreamMessages();
  const polling = new Polling(streamMessages$, app, PollingState, MessagesList, SubscriptionOnStream, PollingActions);
  polling.bindToDOM();
});
;// ./src/index.js
